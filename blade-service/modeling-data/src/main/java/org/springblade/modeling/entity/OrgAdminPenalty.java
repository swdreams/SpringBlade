/**
 * Copyright (c) 2018-2028, Chill Zhuang 庄骞 (smallchill@163.com).
 * <p>
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 * <p>
 * http://www.apache.org/licenses/LICENSE-2.0
 * <p>
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package org.springblade.modeling.entity;

import java.io.Serializable;
import java.math.BigDecimal;
import com.baomidou.mybatisplus.annotation.TableName;
import org.springblade.core.mp.base.BaseEntity;
import com.baomidou.mybatisplus.annotation.TableField;
import lombok.Data;
import lombok.EqualsAndHashCode;
import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;

/**
 * 行政处罚
实体类
 *
 * @author Blade
 * @since 2020-12-16
 */
@Data
@TableName("TB_ORG_ADMIN_PENALTY")
@EqualsAndHashCode(callSuper = false)
@ApiModel(value = "OrgAdminPenalty对象", description = "行政处罚")
public class OrgAdminPenalty implements Serializable {

    private static final long serialVersionUID = 1L;

    /**
     * 机构主ID
     */
    @ApiModelProperty(value = "机构主ID")
    @TableField("ORG_ID")
  private BigDecimal orgId;
    /**
     * 决定日期
     */
    @ApiModelProperty(value = "决定日期")
    @TableField("DECISION_DATE")
  private String decisionDate;
    /**
     * 决定文书号
     */
    @ApiModelProperty(value = "决定文书号")
    @TableField("DECISION_DOC_NO")
  private String decisionDocNo;
    /**
     * 处罚种类
     */
    @ApiModelProperty(value = "处罚种类")
    @TableField("DECISION_TYPE")
  private String decisionType;
    /**
     * 处罚事由
     */
    @ApiModelProperty(value = "处罚事由")
    @TableField("DECISION_SUBJECTMATTER")
  private String decisionSubjectmatter;
    /**
     * 处罚依据
     */
    @ApiModelProperty(value = "处罚依据")
    @TableField("DECISION_REASON")
  private String decisionReason;
    /**
     * 处罚结果
     */
    @ApiModelProperty(value = "处罚结果")
    @TableField("DECISION_RESULT")
  private String decisionResult;
    /**
     * 公开日期
     */
    @ApiModelProperty(value = "公开日期")
    @TableField("DECISION_PUBLIC_DATE")
  private String decisionPublicDate;
    /**
     * 处罚状态
     */
    @ApiModelProperty(value = "处罚状态")
    @TableField("DECISION_STATUS")
  private String decisionStatus;
    /**
     * 决定机关
     */
    @ApiModelProperty(value = "决定机关")
    @TableField("DECISION_ORG_NAME")
  private String decisionOrgName;
  @TableField("ID")
  private BigDecimal id;


}
